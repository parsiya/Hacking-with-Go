...
## Login with SSH key
It's also possible to pass another `AuthMethod` and login with a key. Luckily, the package has [another example][publickey-login-example-ssh-pkg]. Our program is similar to the previous ones. We read the PEM encoded private key and use it in `ClientConfig`.
...
Each session can only run one command. For each new command we need to create a [new session][newsession-ssh-pkg] (the SSH connection can support multiple sessions). We can run commands using one of these methods:
...
3. For obvious reasons, it seems like `CombinedOutput` is the preferred method.
...
Results from my VM (don't get excited, it's the default user/pass for modern.ie VMs):
...
Of course, we can always cheat by separating commands. On Windows use `&` and `&&`.
...
Using `Run` is similar, we buffer `session.Stdout/Stderr` before we execute the command and print them after. Similar to the [package example][run-example-ssh-pkg].
